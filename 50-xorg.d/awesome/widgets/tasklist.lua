...
local function list_update(w, buttons, label, data, objects)
    ...
    for i, o in ipairs(objects) do
        ...
        if cache then
            ...
            tbm = wibox.container.margin(tb, dpi(3), dpi(3))
            ibm = wibox.container.margin(ib, dpi(5), dpi(5), dpi(5), dpi(5))
...
local function tasklist_update(s, w, buttons, filter, data, style, update_function)
    ...
    list_update(w, buttons, label, data, clients)
end
...
function tasklist.new(screen, filter, buttons, style)
    ...
    local w = wibox.layout.flex.horizontal()
